/**
 * Performance is not OK.. But it does the job
 * 
 * Full run takes 1129seconds / 18m 48s
 */
...
const init = async (test) => {
    ...
    const seedRanges = content.match(/seeds: (.*)/m)[1]
    const seedgroups = Array.from(seedRanges.matchAll(/(\d{1,} \d{1,})/gm))
    ...
    seedgroups.forEach((group, index) => {
        const startedGroup = Date.now();
        console.log(`------- Group ${index+1} of ${seedgroups.length}`)
        const range = group[0]
        const parts = range.split(" ")
        const rangeStart = parseInt(parts[0])
        const rangeEnd = rangeStart + parseInt(parts[1])

        console.log(`Walking ${parseInt(parts[1])} seeds`)
        for(let curSeed=rangeStart; curSeed<rangeEnd; curSeed++) {
            const seedLocation = types.reduce((val, type) => mapEntity(type, val), curSeed)
        ...
        console.log(`Walking group took ${Math.floor((Date.now() - startedGroup) / 1000)} seconds \n`)
    });

    console.log(`Total walk took ${Math.floor((Date.now() - startedWalking) / 1000)} seconds`)
